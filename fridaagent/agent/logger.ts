...
export class LogMessage {
    ...
    addTimestampPart() {
        ...
        this.add(new LogMessagePart(LogPartKey.TimestampS, LogPartType.Int64, timestamp / 1000) );
        this.add(new LogMessagePart(LogPartKey.TimestampMS, LogPartType.Int64, timestamp % 1000) );
    }
    pack(): ArrayBuffer {
        ...
        var bufPtr:NativePointer = buf.writeS32( swapInt32(2 + this.partTotoalBytes) ).add(4)
            .writeS16( swapInt16(partNum)).add(2);
        var i:number = 0
        for (i = 0; i < partNum; i++) {
...
export class LogMessagePart {
    ...
    private packTypeValue(bufPtr: NativePointer ,type: LogPartType, value: any):NativePointer {
        var retPtr:NativePointer = bufPtr;
        if (type === LogPartType.String) {
            const str = value as string;
            retPtr = bufPtr.writeS32( swapInt32(str.length)).add(4).writeUtf8String(str).add(str.length);
        } else if (type === LogPartType.Int16) {
            ...
            retPtr = bufPtr.writeS32( swapInt32(arrBufLen)).add(4).writeByteArray(arrBuf).add(arrBufLen);
        } 
        //TODO: Image
        return retPtr;
    }
...
function buildDeviceInfo():ArrayBuffer {
    ...
    msg.add(new LogMessagePart(LogPartKey.MessageType, LogPartType.Int32, LogMsgType.Clientinfo)); //client info
    msg.add(new LogMessagePart(LogPartKey.ClientVersion, LogPartType.String, 'Frida' + Frida.version));
    msg.add(new LogMessagePart(LogPartKey.ClientName, LogPartType.String, 'pid' + Process.id));
    msg.add(new LogMessagePart(LogPartKey.OSName, LogPartType.String, Process.platform + '_' + Process.arch));
...
export class Logger {
    ...
    private tryConnect() {
        if (this.state == LoggerState.Disconnect) {
            ...
            Socket.connect({'family': 'ipv4', 'host': this.host, 'port': this.port, 'tls': false }).then((connect) => {
                this.socket = connect;
    ...
    private send(pkg: ArrayBuffer|undefined) {
        ...
        this.socket.output.write(pkg).then( (val)=>{
    ...
    private buildLog(data: string|ArrayBuffer) {
        var msg = new LogMessage();
        msg.addTimestampPart();
        msg.add(new LogMessagePart(LogPartKey.MessageType, LogPartType.Int32, LogMsgType.Log)); //log message
